...
namespace ZCJ.Utility
{
    ...
    public class Utils
    {
        ...
        /// <summary> 将数据库中查询结果重组成Select所需的List<SelectListItem>数组（重载）</summary>
        /// <returns></returns>
        public static List<SelectListItem> DataReaderToSelect(SqlDataReader readerSource, string valueField, string textField)
        {
            return DataReaderToSelect(readerSource, valueField, textField, "--请选择--");
        }

        /// <summary> 将数据库中查询结果重组成Select所需的List<SelectListItem>数组 </summary>
        /// <param name="readerSource">数据源</param>
        /// <param name="valueField">value值</param>
        /// <param name="textField">text值</param>
        /// <param name="defaultText">默认显示内容</param>
        /// <returns></returns>
        public static List<SelectListItem> DataReaderToSelect(SqlDataReader readerSource, string valueField, string textField, string defaultText)
        {
            using (SqlDataReader reader = readerSource)
            {
        ...
        /// <summary>  asp.net上传图片并生成缩略图  </summary> 
        ...
        public static string UpLoadImage(HttpPostedFileBase upImage, string sSavePath, string sThumbExtension, int intThumbWidth, int intThumbHeight)
        {
            string sThumbFile = "";
            string sFilename = "";
            ...
            {
                string realPath = HttpContext.Current.Server.MapPath(sSavePath);
                ...
                //获取upImage选择文件的扩展名 
                string extendName = Path.GetExtension(upImage.FileName).ToLower();
                //判断是否为图片格式 
                ...
                while (System.IO.File.Exists(realPath + sFilename))
                {
                    file_append++;
                ...
                System.IO.FileStream newFile
                    = new System.IO.FileStream(realPath + sFilename,
                ...
                {
                    //原图加载 
                    using (Image sourceImage = Image.FromFile(realPath + sFilename))
                    {
                        //原图宽度和高度 
                        ...
                        while (System.IO.File.Exists(realPath + sThumbFile))
                        {
                            file_append++;
                        ...
                        //缩略图保存的绝对路径 
                        string smallImagePath = realPath + sThumbFile;
                ...
                {
                    //出错则删除 
                    System.IO.File.Delete(realPath + sFilename);
                    //return "图片格式不正确";
                    return "";
                }
                //返回缩略图名称 
        ...
        /// <param name="sSavePath">保存的路径,些为相对服务器路径的下的文件夹</param>
        /// <returns>缩略图名称</returns> 
        public static string UpLoadImage(HttpPostedFileBase upImage, string sSavePath)
        {
            string sThumbFile = "";
            string sFilename = "";
            ...
            if (upImage.FileName != null)
            {
                ...
                {
                    file_append++;
                    sFilename = System.IO.Path.GetFileNameWithoutExtension(upImage.FileName) + file_append.ToString() + extendName;
                }

                System.IO.FileStream newFile = new System.IO.FileStream(realPath + sFilename, System.IO.FileMode.Create, System.IO.FileAccess.Write);
                newFile.Write(myData, 0, myData.Length);
                newFile.Close();
                ...
                //返回缩略图名称 
                return sSavePath + upImage.FileName;
            }
            //return "没有选择图片";
            return "";
        }
